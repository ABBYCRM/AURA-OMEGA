...
import { CheckCircle2, ChevronDown, ExternalLink, KeyRound, Lock, PlugZap, RefreshCw, Save, Search, ShieldCheck, UserRoundCog, X, AlertTriangle } from "lucide-react";
...
export default function IntegrationsConsole() {
  ...
  const [toolkits, setToolkits] = useState<Array<{ slug: string; name: string; logo?: string; authSchemes?: string[]; requiresAuthConfig?: boolean; setupUrl?: string }>>([]);
  ...
  async function openToolkitBrowser() {
    ...
    if (toolkits.length === 0 && !toolkitsLoading) {
      ...
      try {
        const out = await apiJson<{ toolkits: Array<{ slug: string; name: string; logo?: string; authSchemes?: string[]; requiresAuthConfig?: boolean; setupUrl?: string }> }>("/api/integrations/composio/toolkits?limit=200");
  ...
  async function connectFromDropdown(slug: string) {
    ...
    try {
      ...
      // Operator directive 2026-06-27: a 409 with needsSetup=true means the
      // app requires manual OAuth registration in the Composio dashboard.
      // Show a clear message + open the dashboard link in a new tab.
      const msg = String(err);
      const needsSetupMatch = /needsSetup/i.test(msg) || /auth[_-]?config/i.test(msg);
      if (needsSetupMatch) {
        setOauthResult({ needsSetup: true, toolkit: slug, error: msg });
        setStatus(`${slug} needs manual setup. Click the "Setup" link in the apps list to register it in Composio.`);
      } else {
        setStatus(`OAuth failed for ${slug}: ${msg}`);
      }
    } finally {
