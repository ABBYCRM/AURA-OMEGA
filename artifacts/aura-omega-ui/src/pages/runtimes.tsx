...
const LANE_DESTINATION: Record<string, string> = {
  "BOS-OMEGA Governor": "/swarm",
  "Kimi K2.6 Brain": "/integrations",
  "Tool Selection Matrix": "/tools",
  "n8n Executor": "/scheduled",
  "Heartbeat Autonomy": "/swarm",
  "Memory Lattice": "/hermes",
  "Browser/Scrape Lane": "/remote",
  "Code/Deploy Lane": "/missions",
};
...
export default function RuntimesPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{runtimeLanes.map((lane, i) => { const Icon = icons[i % icons.length]; const dest = LANE_DESTINATION[lane.name] ?? "/"; return <div key={lane.name} className="rounded-2xl border border-card-border bg-card/50 p-5"><div className="flex items-center justify-between"><Icon className="w-6 h-6 text-primary" /><span className="rounded-full border border-card-border px-2 py-1 text-[10px] font-bold uppercase text-muted-foreground">{lane.status}</span></div><h3 className="mt-4 font-black">{lane.name}</h3><p className="mt-1 text-sm text-muted-foreground">{lane.role}</p><button onClick={() => setLocation(dest)} aria-label={`Open ${lane.name}`} className="mt-4 inline-flex items-center gap-2 rounded-xl border border-card-border px-3 py-2 text-xs font-bold hover:border-primary hover:text-primary transition-colors"><PlayCircle className="w-4 h-4" />Open lane</button></div> })}</section>
