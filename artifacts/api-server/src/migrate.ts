...
const CRON_JOBS_SCHEMA_SQL = MISSION_SCHEMA_SQL + `
-- Cron jobs (operator doctrine 2026-06-27: schedule via the kernel, not via n8n).
-- The 60 n8n-* workflow definitions live as rows here, get scheduled via
-- setInterval at boot, and execute as long-running missions that the runtime
-- re-ticks on every interval. Postgres-backed so a restart doesn't lose the
-- schedule or the last_run_at timestamps.
CREATE TABLE IF NOT EXISTS "mission_cron_jobs" (
  "id" serial PRIMARY KEY NOT NULL,
...
CREATE INDEX IF NOT EXISTS "mission_cron_jobs_enabled_idx" ON "mission_cron_jobs" ("enabled");
CREATE INDEX IF NOT EXISTS "mission_cron_jobs_next_run_idx" ON "mission_cron_jobs" ("enabled", "last_run_at");
`;
...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    await client.query(CRON_JOBS_SCHEMA_SQL);
    logger.info("Schema ready (core + hermes + openhands + crawl4ai + mem0 + docling + bos + mission + cron)");
